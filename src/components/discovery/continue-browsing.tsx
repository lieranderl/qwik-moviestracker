						...
						<div class="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
							{lastViewed.value ? (
								<a
									...
									class="media-card-link rounded-box border-base-200 bg-base-200/45 card-hover flex items-center gap-4 border p-4"
								>
									<div class="bg-primary/12 text-primary flex h-12 w-12 items-center justify-center rounded-2xl">
										<HiPlaySolid class="h-5 w-5" />
									</div>
									<div class="min-w-0 flex-1">
										<p class="text-xs font-semibold tracking-[0.12em] uppercase text-base-content/50">
								...
								<div class="rounded-box border-base-200 bg-base-200/35 flex items-center gap-3 border p-4 text-sm text-base-content/65">
									<HiArrowTrendingUpSolid class="h-5 w-5 text-primary" />
							...
							<div class="rounded-box border-base-200 bg-base-200/30 border p-4">
								<div class="mb-3 flex items-center gap-2 text-sm font-semibold">
									<HiMagnifyingGlassSolid class="h-4 w-4 text-primary" />
									...
									{recentSearches.value.length > 0 ? (
										recentSearches.value.map((search) => (
											<a
												key={search.href}
												href={search.href}
												class="btn btn-ghost btn-sm rounded-full border-base-200 bg-base-100/70 border"
											>
												{search.query}
											</a>
										))
